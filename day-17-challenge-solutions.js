...
const ADMIN_PERMISSIONS = ["read", "write", "delete", "manage_users"];
//3. Create a `readonly string[]` called `EDITOR_PERMISSIONS` with 2 permissions
const EDITOR_PERMISSIONS = ["read", "write"];
...
function canPerform(role, action, permissions) {
...
function printLeaderboard2(entries) {
    entries.forEach(([rank, playerName, score]) => {
        console.log(`#${rank} -- ${playerName}: ${score} pts`);
    });
}
...
//Array Destructuring
const singleRow = [1, 'Kingsley', 'king@dev', 25, true];
...
function getRow2({ matrix, rowIndex }) {
    return matrix[rowIndex];
}
...
export {};
//# sourceMappingURL=day-17-challenge-solutions.js.map